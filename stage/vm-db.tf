resource "aws_instance" "db_stage" {
...
resource "aws_eip" "eip_db_stage" {
  instance = aws_instance.db_stage.id
  domain   = "vpc"
}
...
resource "aws_iam_role_policy_attachment" "ecr_read_only_policy_additional" {
